...
var drawOrbs = (functionContext) => {
  ...
  points.start.forEach((start, i) => {
    ...
    helper.drawCircle(points.end[i], h * 0.01, {
...
var drawRings = (functionContext) => {
  ...
  let outerCircle = helper.getPoints(
    "circle",
    minDimension / 2,
    [w / 2, h / 2],
    data[0].length,
  ...
  let innerCircle = helper.getPoints(
    "circle",
    minDimension / 4,
    [w / 2, h / 2],
    data[1].length,
  ...
  let largerInner = (data[1] = helper.mutateData(
    data[1],
    "scale",
    minDimension / 4 - minDimension / 2
  ));
  let innerBars = helper.getPoints(
    "circle",
    middle,
    [w / 2, h / 2],
    data[1].length,
...
var drawShineRings = (functionContext) => {
  ...
  let outerBars = helper.getPoints(
    "circle",
    minDimension / 2,
    [w / 2, h / 2],
    data.vocals.length,
  ...
  let innerWave = helper.getPoints(
    "circle",
    minDimension / 2,
    [w / 2, h / 2],
    data.vocals.length,
...
var drawShockwave = (functionContext) => {
  ...
  let colorIndex = 0;
  data.forEach((points) => {
    let wavePoints = helper.getPoints(
      "line",
      w,
      [0, h / 2],
      points.length,
...
var drawWeb = (functionContext) => {
  ...
  points.end.forEach((end, i) => {
    helper.drawCircle(end, minDimension * 0.01, {
      color: colors[1],
  ...
  (function (window) {
    ...
    Origami.styles = function () {
      ...
      for (var i = 0; i < selectors.length; i++) {
        var style = styleRuleValueFrom(
          selectors[i],
          config.documentStyles[0] || []
        );
    ...
    function argsByRules(argsArray, rules) {
      ...
      for (var i = 0; i < argsArray.length; i++) {
        if (typeof argsArray[i] === "object") args["style"] = argsArray[i];
        else if (params.length) args[params.shift()] = argsArray[i];
      }
    ...
    function normalizeStyle(style) {
      ...
      if (typeof borderStyle === "string") {
        if (borderStyle === "dashed") borderStyle = [12];
        else if (borderStyle === "dotted") borderStyle = [3];
        else borderStyle = [];
      }
    ...
    function smartCoordinates(args) {
      ...
      if (axis.x.indexOf(x) !== -1) {
        if (x === "right") x = Math.floor(elmWidth - width);
        else if (x === "center")
          if (radius) x = Math.floor(elmWidth / 2);
          else x = Math.floor(elmWidth / 2 - width / 2);
      ...
      if (axis.y.indexOf(y) !== -1) {
        if (y === "top") y = radius;
          ...
          if (radius) y = Math.floor(elmHeight / 2);
          else y = Math.floor(elmHeight / 2 - height / 2);
    ...
    function styleRuleValueFrom(selector, documentStyleRules) {
      for (var j = 0; j < documentStyleRules.length; j++) {
        if (
          documentStyleRules[j].selectorText &&
          documentStyleRules[j].selectorText.toLowerCase() === selector
        ) {
    ...
    function ArcShape(params) {
      ...
      this.paper.ctx.arc(
        ...
        args.r || def.radius,
        args.sAngle || 0,
        args.eAngle || 2 * Math.PI
      );
      this.paper.ctx.fillStyle =
      ...
      this.paper.ctx.lineWidth = style.borderSize
        ? style.borderSize
        : def.lineWidth;
      this.paper.ctx.strokeStyle = style.borderColor
        ? style.borderColor
        : def.strokeStyle;
    ...
    function PolygonShape(params) {
      ...
      for (var p = 0; p < args.length; p++) {
        ...
        if (p) this.paper.ctx.lineTo(args[p].x, args[p].y);
        else this.paper.ctx.moveTo(args[p].x, args[p].y);
      }
    ...
    function drawSprite(sprite) {
      ...
      setTimeout(function () {
        sprite.animation = window.requestAnimationFrame(
          drawSprite.bind(self, sprite)
        );
      }, sprite.speed);
    }
    ...
    function ChartLine(config) {
      ...
      if (config.points) {
        ctx.fillStyle = config.pointsColor
          ? config.pointsColor
          : "rgb(75,75,75)";
        for (var i = 0; i < data.length; i++) {
          ctx.beginPath();
    ...
    Origami.rotate = function (degrees) {
      ...
      if (typeof degrees === "string") {
        ...
        if (degrees === "slow")
          degrees =
            ((2 * Math.PI) / 60) * new Date().getSeconds() +
            ((2 * Math.PI) / 60000) * new Date().getMilliseconds();
        // Normal
        else if (degrees === "normal")
          degrees =
            ((2 * Math.PI) / 30) * new Date().getSeconds() +
